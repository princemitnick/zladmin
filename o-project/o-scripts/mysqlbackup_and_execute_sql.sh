...
mysqldump -u${USER} -p${PASS} --opt --flush-logs --single-transaction --master-data=2 openmrs 2>/dev/null | 7za a -p${PASS} -siy -t7z ${DIR}/sequences/${FILENAME} -mx9 2>&1 >/dev/null
